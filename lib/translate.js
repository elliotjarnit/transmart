...
async function translate(params, retryTime = 0) {
    const { baseLang, targetLang, openAIApiKey, retryTimes, openAIApiUrl, openAIApiUrlPath, content, openAIApiModel, context, systemPromptTemplate, additionalReqBodyParams, requestsPerMinuteLimit, } = params;
    const headers = {
...
The aim is to achieve a fluent and structurally sound translation of the JSON content from the base language to the target language ${languageName}, without altering the document's schema or disrupting the key-value relationship.`;
    ...
    await (0, limit_1.requestRateLimit)(requestsPerMinuteLimit);
