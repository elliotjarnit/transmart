...
async function translate(params, retryTime = 0) {
    const { baseLang, targetLang, openAIApiKey, retryTimes, openAIApiUrl, openAIApiUrlPath, content, openAIApiModel, context, systemPromptTemplate, additionalReqBodyParams, requestsPerMinuteLimit, tokensPerMinuteLimit, } = params;
    const headers = {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${openAIApiKey}`,
    };
    const languageName = (0, language_1.getLanguageDisplayName)(targetLang);
...
${context
            ...
            : ''}
- **Address plural forms** specifically by applying the appropriate suffixes "${getPluralSuffixes(baseLang)}" from the base language to the corresponding "${getPluralSuffixes(targetLang)}" suffixes in the target language ${languageName}. Accurate plural translations must be provided based on these suffixes.
...
const findValidJSONInsideBody = (input) => {
    const firstBracket = input.indexOf('{');
    const lastBracket = input.lastIndexOf('}') + 1;
    return input.substring(firstBracket, lastBracket);
};
const getPluralSuffixes = (input) => new Intl.PluralRules(input)
    .resolvedOptions()
    .pluralCategories.map((cat) => `_${cat}`)
    .join(', ');
const getRequestTokenCount = (body) => {
    const reservedCompletionTokens = typeof body.max_tokens === 'number' ? body.max_tokens : 0;
    return (0, gpt_3_encoder_1.encode)(JSON.stringify(body)).length + reservedCompletionTokens;
};
