...
class RequestRateLimiter {
    ...
    async waitForSlot(options) {
        ...
        while (!isScheduled) {
            ...
            const requestSlotAvailable = requestsPerMinuteLimit === undefined || this.requestTimestamps.length < requestsPerMinuteLimit;
            const tokenSlotAvailable = tokensPerMinuteLimit === undefined ||
    ...
    getWaitTime(now, requestsPerMinuteLimit, tokensPerMinuteLimit, tokenCount = 0) {
        const waitTimes = [];
...
const requestRateLimit = (options) => requestRateLimiter.wait(options);
